...
if [ -z "$key" ]; then
    ...
    dotnet user-secrets set JwtIssuerOptions:Secret ${key} -p "./Avocado.Web"
elif [ -n "$key" ]; then
...
jwt=$(dotnet user-secrets list -p "./Avocado.Web" | grep "$key")
if [ -n "$jwt" ]; then
...
if [[ "$ans" =~ ^[yY]e?s?$ ]]; then
    ...
    if [ -f "./firebase.json" ]; then
        ...
        cat ./firebase.json | jq 'tostring' | sed -e 's/"{/{ "FirebaseJson": "{/' | sed -e 's/}"/}"}/' | jq | dotnet user-secrets set -p "./Avocado.Web"
    else
    ...
    if dotnet user-secrets list -p "./Avocado.Web" | grep "FirebaseJson"; then
        echo "Key saved successfully."
    else
        echo "Key was not saved successfully. Please check key and try again."
        exit 1
